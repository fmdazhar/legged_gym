...
if __name__ == '__main__':
    ...
    for adjacent in adjacents:
        inds = adjacent.nonzero()[0]
        # Wrap rewards and thresholds into lists
        task_rewards = [torch.ones(len(inds)), torch.ones(len(inds))]
        thresholds = [0.0, 0.0]
        r.update(bin_inds=inds, task_rewards=task_rewards, success_thresholds=thresholds, local_range=0.5)
